...
export interface DropdownProps {
  ...
  className?: string;
  // ─── v1.1.0 New Props ───
  /** Show "Select All / Deselect All" toggle in multi-select */
  showSelectAll?: boolean;
  /** Max tags to display before showing "+N more" chip */
  tagLimit?: number;
  /** Allow creating new options via search input */
  creatable?: boolean;
  /** Callback when a new option is created */
  onCreateOption?: (label: string) => void;
  /** Placeholder for create option prompt */
  createLabel?: string;
  /** Custom empty state message or ReactNode */
  emptyState?: React.ReactNode;
  /** Footer content rendered at bottom of dropdown panel */
  footer?: React.ReactNode;
  /** Async search callback — fires on search input change with debounce */
  onSearch?: (query: string) => void;
  /** Debounce delay in ms for onSearch (default: 300) */
  searchDebounce?: number;
  /** Show loading spinner inside the dropdown list (for async search) */
  searchLoading?: boolean;
  /** Render a custom option node (overrides default rendering) */
...
export const Dropdown = forwardRef<HTMLDivElement, DropdownProps>(
  (
    ...
    const filteredOptions = useMemo(() => {
      if (!search || onSearch) return options; // if async, don't filter locally
    ...
    // v1.1.0: Select All / Deselect All
    ...
    const handleSelectAll = useCallback(() => {
      ...
      if (allSelected) {
        ...
        // Select all filtered (respect maxSelections)
        const arr = Array.isArray(value) ? [...value] : [];
    ...
    return (
      <div ref={ref} className={`relative w-full ${className}`} id={id}>
        ...
        >
          {loading ? (
            <Loader2 size={sc.icon} className="animate-spin text-muted-foreground" />
          ) : null}

          <span className={`flex-1 text-left truncate ${!hasValue ? "text-muted-foreground" : "text-foreground"}`}>
            {hasValue ? (
              multiple ? renderTags() : selectedLabels[0]
            ) : (
              placeholder
            )}
          </span>
        ...
        {isOpen && createPortal(
          ...
          >
              ...
              {searchLoading ? (
                ...
                emptyState ? (
                  ...
                  <div className="text-center py-6 text-muted-foreground flex flex-col items-center gap-2">
                    ...
                    <span style={{ fontFamily: "var(--font-label)", fontSize: "var(--text-label)" }}>
                      No options found
                    </span>
                  </div>
                )
                ...
                Object.entries(groupedOptions).map(([group, opts]) => (
                  <div key={group}>
                    ...
                    {opts.map((opt) => {
                      const node = renderOption(opt, globalIdx);
                      globalIdx++;
                      return node;
                    })}
                  </div>
            ...
            {/* Footer — maxSelections or custom (v1.1.0) */}
