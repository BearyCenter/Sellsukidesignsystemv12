...
function Rating({
  value = 0, onChange, max = 5, size = "md", disabled, readOnly, icon = "star", showValue, label, half,
}: {
  value?: number; onChange?: (v: number) => void; max?: number; size?: "sm" | "md" | "lg";
  disabled?: boolean; readOnly?: boolean; icon?: "star" | "heart" | "thumb";
  showValue?: boolean; label?: string; half?: boolean;
}) {
  ...
  return (
    <div className={`inline-flex items-center ${gap} ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
      {label && <span className="text-foreground mr-2" style={fontLabel}>{label}</span>}
      {items.map(n => {
        const filled = n <= (hover || value);
        return (
          <button key={n} type="button"
            className={`transition-transform ${readOnly || disabled ? "" : "cursor-pointer hover:scale-110"} ${filled ? activeColor : "text-muted-foreground/40"}`}
            onMouseEnter={() => !readOnly && !disabled && setHover(n)}
            onMouseLeave={() => setHover(0)}
            onClick={() => !readOnly && !disabled && onChange?.(n === value ? 0 : n)}
            disabled={disabled || readOnly}
          >
            <IconComp size={iconSize} fill={filled ? "currentColor" : "none"} />
          </button>
        );
      })}
      {showValue && <span className="text-foreground ml-1" style={fontLabelBold}>{value}/{max}</span>}
    </div>
  );
}
